...
public class CustomAdminLoginController {
    ...
    public ResponseEntity<Map<String, Object>> sendOtpMobile(@RequestBody MobileNoDTO otpRequest) {
        ...
        Optional<CustomAdmin> admin = customAdminRepository.findByMobileNumber(mobile);
        if (admin.isEmpty()) {
    ...
    public ResponseEntity<Map<String, Object>> verifyOtpMobile(@RequestBody Map<String, String> request) {
        ...
        try {
            ...
            Long userId = otpService.getCustomAdminByMobile(mobileNumber); 

            String token = jwtService.generateToken(mobileNumber, "customAdmin", userId);
