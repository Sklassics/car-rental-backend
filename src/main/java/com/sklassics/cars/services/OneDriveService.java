...
import java.util.*;
	...
	public class OneDriveService {
	    ...
	    public String uploadFile(MultipartFile file, String userFolder, String fileName) throws Exception {
	        GraphServiceClient<Request> graphClient = getGraphClient();

	        String rootFolder = "Car";
	        String targetFolderPath = rootFolder + "/" + userFolder;

	        // Ensure the folder "Car/userFolder" exists
	        createFolderIfNotExists(graphClient, rootFolder, userFolder);
	        ...
	        // Define the full path to upload the file
	        String fullUploadPath = String.format("%s/%s", targetFolderPath, fileName + fileExtension);

	        // Upload the file
	        graphClient.users(userEmail)
	                .drive()
	                .root()
	                .itemWithPath(fullUploadPath)
	                .content()
	                .buildRequest()
	                .put(file.getBytes());
