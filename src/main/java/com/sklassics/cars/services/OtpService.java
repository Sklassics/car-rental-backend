...
public class OtpService {
	...
	public ResponseEntity<?> validateMobileEmailOtp(String email, String otp) {
		...
		if (cached == null) {
			return ResponseEntity.badRequest().body(ResponseUtil.notFound("No OTP generated for this email."));
		}
		...
		// Generate token
		String token = jwtService.generateToken(cached.getMobile(), "customer", user.getId()); // Use cached.getMobile()
																								// to fetch the correct
																								// mobile number
	...
	public ResponseEntity<?> validateLoginOtp(String email, String otp) {
		...
		if (email == null || email.trim().isEmpty()) {
			return ResponseEntity.badRequest().body(ResponseUtil.conflict("Email must not be empty."));
		}

		if (otp == null || otp.trim().isEmpty()) {
			return ResponseEntity.badRequest().body(ResponseUtil.conflict("OTP must not be empty."));
		}

		// OTP format validation (assuming 6-digit numeric)
		if (!otp.matches("^\\d{6}$")) {
			return ResponseEntity.badRequest().body(ResponseUtil.conflict("Invalid OTP format. OTP must be 6 digits."));
		}
...
//	}
	...
	public void registerUser(String name, String address, String aadharNumber, MultipartFile aadharFile,
			MultipartFile drivingLicenseFile, Long userId, MultipartFile selfieFile) {
		...
		String selfieUrl = saveFileToOneDrive(selfieFile, "selfies");
		System.out.println("Aadhaar file uploaded to OneDrive at: " + selfieUrl);
	...
	public void customRegisterUser(String name, String address, String aadharNumber, MultipartFile aadharFile,
			MultipartFile drivingLicenseFile, Long userId, MultipartFile selfieFile) {
